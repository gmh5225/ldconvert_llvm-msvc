...
myclassify(long double ld)
{
    ...
    case 0: // Exponent: All Zeros
    {
        ...
        {
            return FltDenormal; // Pseudo Denormal. The 80387 and later properly interpret this value but will not
                                // generate it. The value is (−1)^s * m * 2^(−16382)
        }
    }
    ...
    case 0x7fff: // Exponent: All Ones
    {
        ...
        if (!bit63 && !bit62) // Bits 63,62: 00
        {
            ...
            {
                return FltInvalid; // Pseudo-Infinity. The sign bit gives the sign of the infinity. The 8087 and 80287
                                   // treat this as Infinity. The 80387 and later treat this as an invalid operand.
            }
            else // Bits 61-0: Non-zero
            {
                return FltInvalid; // Pseudo Not a Number. The sign bit is meaningless. The 8087 and 80287 treat this as
                                   // a Signaling Not a Number. The 80387 and later treat this as an invalid operand.
            }
        }
        else if (!bit63 /* && bit62*/) // Bits 63,62: 01
        {
            // Bits 61-0: Anything
            return FltInvalid; // Pseudo Not a Number. The sign bit is meaningless. The 8087 and 80287 treat this as a
                               // Signaling Not a Number. The 80387 and later treat this as an invalid operand.
        }
        else if (/*bit63 && */ !bit62) // Bits 63,62: 10
        {
            if (bit61to0_zero) // Bits 61-0: Zero
            {
                return FltInfinity; // Infinity. The sign bit gives the sign of the infinity. The 8087 and 80287 treat
                                    // this as a Signaling Not a Number. The 8087 and 80287 coprocessors used the
                                    // pseudo-infinity representation for infinities.
            }
            else // Bits 61-0: Non-zero
            {
        ...
        else /*if (bit63 && bit62)*/ // Bits 63,62: 11
        {
            if (bit61to0_zero) // Bits 61-0: Zero
            {
                return FltQNaN; // Floating-point Indefinite, the result of invalid calculations such as square root of
                                // a negative number, logarithm of a negative number, 0/0, infinity / infinity, infinity
                                // times 0, and others when the processor has been configured to not generate exceptions
                                // for invalid operands. The sign bit is meaningless. This is a special case of a Quiet
                                // Not a Number.
            }
            else // Bits 61-0: Non-zero
            {
                return FltQNaN; // Quiet Not a Number, the sign bit is meaningless. The 8087 and 80287 treat this as a
    ...
    default: // Exponent: All other values
    {
        if (!bit63) // Bit 63: Zero
        {
            return FltInvalid; // Unnormal. Only generated on the 8087 and 80287. The 80387 and later treat this as an
                               // invalid operand. The value is (−1)^s * m * 2^(e−16383)
        }
        else // Bit 63: One
        {
...
extern "C" __declspec(dllexport) void ld2str(const void *pld, char *str)
{
    ...
    case FltInvalid:
        ...
        break;
    default:
        snprintf(str, 32, "%#.*Lg", LDBL_DIG + 1, ld);
    }
}
